...
// Signin: expects { username, password }
router.post("/signin", (req, res) => {
  const { username, password } = req.body || {}
  if (!username || !password) return res.status(400).json({ error: "WRONG INFO" })
  const users = readUsers()
  const user = users.find((u) => u.username === username)
  if (!user || user.password !== password) return res.status(400).json({ error: "WRONG INFO" })
